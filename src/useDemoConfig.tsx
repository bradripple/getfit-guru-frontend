  ...
  React.useEffect(() => {
    ...
    const fetchProgress = async () => {
      const response = await Axios.get(`${REACT_APP_SERVER_URL}/progress`);
         console.log("fetch progress", response);
      // const { results } = response.data.routines;
      let exerciseDate = new Date(response.data.progress[0].date); 
      var options = { weekday: 'long'};
      let progressData = response.data.progress.map( async (p, i) => {
        let exerciseDate = new Date(p.date);
        let dayOfWeek = new Intl.DateTimeFormat('en-US', options).format(exerciseDate.getDay());
        let month = exerciseDate.getMonth();
        let dayOfMonth = exerciseDate.getDate();
        let weight = p.weight;
